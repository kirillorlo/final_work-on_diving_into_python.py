...
class Student:
    ...
    def init(self, name, subjects_file):
        logging.info(f"Инициализация студента: {name}")
        self.name = name
        self.subjects = {}
        self.load_subjects(subjects_file)

    def setattr(self, name, value):
        logging.info(f"Установка атрибута: {name} = {value}")
        if name == 'name':
            if not value.replace(' ', '').isalpha() or not value.istitle():
                raise ValueError("ФИО должно состоять только из букв и начинаться с заглавной буквы")
    ...
    def getattr(self, name):
        logging.info(f"Получение атрибута: {name}")
        if name in self.subjects:
            return self.subjects[name]
        else:
            raise AttributeError(f"Предмет {name} не найден")
    ...
    def str(self):
        logging.info("Получение строкового представления студента")
        return f"Студент: {self.name}\nПредметы: {', '.join(self.subjects.keys())}"
    ...
    def add_grade(self, subject, grade):
        logging.info(f"Добавление оценки по предмету: {subject} - {grade}")
        if subject not in self.subjects:
            self.subjects[subject] = {'grades': [], 'test_scores': []}
        if not isinstance(grade, int) or grade < 2 or grade > 5:
            raise ValueError("Оценка должна быть целым числом от 2 до 5")
    ...
    def add_test_score(self, subject, test_score):
        logging.info(f"Добавление результата теста по предмету: {subject} - {test_score}")
        if subject not in self.subjects:
            self.subjects[subject] = {'grades': [], 'test_scores': []}
        if not isinstance(test_score, int) or test_score < 0 or test_score > 100:
            raise ValueError("Результат теста должен быть целым числом от 0 до 100")
    ...
    def get_average_test_score(self, subject):
        logging.info(f"Вычисление среднего балла по тестам для предмета: {subject}")
        if subject not in self.subjects:
            raise ValueError(f"Предмет {subject} не найден")
        test_scores = self.subjects[subject]['test_scores']
        if len(test_scores) == 0:
    ...
    def get_average_grade(self):
        logging.info("Вычисление среднего балла по всем предметам")
        total_grades = []
        for subject in self.subjects:
            grades = self.subjects[subject]['grades']
            if len(grades) > 0:
